...
function ProductDetails({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true); // Jab fetching start ho, loading true karo
      setError(false);
      ...
      try {
        ...
        setLoading(false); // Jab data load ho jaye ya error aaye, loading false karo
      }
    };
    ...
    const subscription = client.listen(query, { id: params.id }).subscribe((update) => {
              ...
              <div className="inline-flex gap-x-5">
                ...
                {data.discountPercentage > 0 ? (
                  <>
                    ...
                    <span style={{ textDecoration: "line-through" }} className="text-[16px] text-[#f83434]">
                      ${data.price}
                    </span>
